...
import { LEGACY_PREFIXES, Proxy, UPSTREAM_RULES } from '../src/proxy/index.js';
...
describe('Proxy', () => {
  ...
  before(async () => {
    ...
    proxy = new Proxy({
      ...
      legacyPrefixes: testLegacyPrefixes(targetPort),
    });
    await proxy.start();
  });
  ...
  it('forwards requests to the target and returns the response', async () => {
    const res = await makeAnthropicRequest(proxy.port, '/v1/messages', {
      model: 'claude-test',
  ...
  it('calls onCapture with request and response data', async () => {
    ...
    await makeAnthropicRequest(capturingProxy.port, '/v1/messages', {
      model: 'claude-test',
      messages: [{ role: 'user', content: 'hello' }],
    });
  ...
  it('returns 502 when target is unreachable', async () => {
    ...
    const res = await makeAnthropicRequest(unreachableProxy.port, '/v1/messages', {
      model: 'test',
    });
  ...
  it('routes by header and passes path through unchanged', async () => {
    ...
    await makeAnthropicRequest(proxy.port, '/v1/messages', { model: 'claude-test' });
  ...
  it('sets capture host to upstream hostname', async () => {
    ...
    await makeAnthropicRequest(headerProxy.port, '/v1/messages', { model: 'claude-test' });
...
describe('Proxy resilience', () => {
  ...
  it('returns 504 when upstream never responds (TTFB timeout)', async () => {
    ...
    const hang = await createFakeUpstream(() => {
      /* silence */
    });
