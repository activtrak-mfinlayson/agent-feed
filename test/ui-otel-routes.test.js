...
async function seedEvents(db, sessionId) {
  const seed = [
    { id: 'e1', sequence: 1, kind: 'user_prompt' },
    {
      ...
      sequence: 2,
      kind: 'tool_decision',
      attrs: { tool_name: 'Bash', decision: 'accept' },
    },
  ...
  it('GET /api/sessions/:id/events?kind=tool_decision filters', async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:${port}/api/sessions/sess-events-2/events?kind=tool_decision`,
      );
      const events = await res.json();
  ...
  it('default GET coalesces by request_id (one row per turn)', async () => {
    ...
    try {
      await db.insertRecord(
        baseRecord({ source: 'proxy', request_id: 'r1', raw_response: 'PROXY' }),
      );
      await db.insertRecord(
        baseRecord({ source: 'otel', request_id: 'r1', raw_response: 'OTEL', token_count: 99 }),
      );
